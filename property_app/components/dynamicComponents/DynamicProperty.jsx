...
export default function DynamicProperty({ property }) {
  const data = property || {};
  const typeLabel = typeof data.type === "string" ? data.type : "Listing";
  const nameLabel = typeof data.name === "string" ? data.name : "Property";
  const street = typeof data.location?.street === "string" ? data.location.street : "";
  const city = typeof data.location?.city === "string" ? data.location.city : "";
  const country =
    typeof data.location?.country === "string" ? data.location.country : "";
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-white pb-24 pt-14 font-sans text-slate-900 selection:bg-blue-100 sm:pb-20 md:pt-20">
      <main className="mx-auto max-w-7xl space-y-4 px-4 py-3 sm:space-y-8 sm:px-6 sm:py-6">
        <PropertyImageGallery images={data.images} propertyName={nameLabel} />
