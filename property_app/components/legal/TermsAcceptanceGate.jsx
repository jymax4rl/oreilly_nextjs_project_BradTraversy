...
export default function TermsAcceptanceGate({ lang = "en", onAccepted }) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-[400px] flex-1 flex-col justify-center pt-10 lg:pt-0">
      ...
      <h1 className="mt-3 text-center text-[1.65rem] font-semibold tracking-tight text-zinc-900 lg:text-left lg:text-3xl">
        {t.title}
      </h1>
      <p className="mt-3 text-center text-sm leading-relaxed text-zinc-500 lg:text-left">
        {t.body}
      </p>

      <label className="mt-8 flex cursor-pointer items-start gap-3 rounded-xl border border-zinc-200 bg-white px-4 py-4 shadow-sm">
        <input
          type="checkbox"
          checked={checked}
      ...
      <p className="mt-6 text-center text-xs leading-relaxed text-zinc-400 lg:text-left">
        {t.disclaimer}
      </p>
    </div>
  );
