...
const SHARED_VARIABLES = [
  ...
  /** Plain "Ref #123456" — always include in Resend dashboard templates. */
  { key: "RESERVATION_REFERENCE", type: "string", fallbackValue: "Ref #—" },
  /** Alias for RESERVATION_REFERENCE (dashboard templates may use either key). */
  { key: "reservation_reference", type: "string", fallbackValue: "Ref #—" },
  { key: "RESERVATION_REFERENCE_HTML", type: "string", fallbackValue: "" },
...
/**
 * Stacked label/value row — mobile-safe (no fixed-width columns that clip refs).
 * @param {{ label: string; value: string }} row
 */
export function bookingDetailRowHtml({ label, value }) {
  return `
    <tr>
      <td style="padding:12px 0;border-bottom:1px solid #f3f4f6;font-family:Arial,Helvetica,sans-serif;vertical-align:top;">
        <div style="font-size:12px;line-height:16px;color:#6b7280;font-weight:700;text-transform:uppercase;letter-spacing:0.04em;margin:0 0 4px;">${escapeHtml(label)}</div>
        <div style="font-size:16px;line-height:24px;color:#111827;font-weight:600;word-break:break-word;overflow-wrap:anywhere;">${value}</div>
      </td>
    </tr>`;
}

/**
 * Full-width reservation reference banner (always visible; no media-query hide).
 * Pass already-formatted label e.g. "Ref #10461903".
 */
export function reservationReferenceBannerHtml(refLabel) {
  const safe =
    refLabel && String(refLabel).trim() && String(refLabel).trim() !== "—"
      ? escapeHtml(String(refLabel).trim())
      : null;
  const display = safe || "Not available";
  return `
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="width:100%;max-width:100%;margin:20px 0 0;background-color:#ecfdf5;border:2px solid #1b5c57;border-radius:10px;">
      <tr>
        <td style="padding:16px 18px;font-family:Arial,Helvetica,sans-serif;">
          <div style="font-size:12px;line-height:16px;font-weight:700;color:#1b5c57;text-transform:uppercase;letter-spacing:0.06em;margin:0 0 8px;">Reservation reference</div>
          <div style="font-size:22px;line-height:30px;font-weight:800;color:#0f172a;letter-spacing:0.02em;word-break:break-word;overflow-wrap:anywhere;">${display}</div>
          <div style="font-size:13px;line-height:18px;color:#334155;margin:8px 0 0;">Quote this Ref # when contacting support or your host.</div>
        </td>
      </tr>
    </table>`;
}
...
<body style="margin:0;padding:0;background-color:#f3f4f6;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;-webkit-font-smoothing:antialiased;">
  <!-- Preview text only; trailing ZWNJ/nbsp stops Gmail from clipping the body on mobile. -->
  <div style="display:none;font-size:1px;color:#f3f4f6;line-height:1px;max-height:0;max-width:0;opacity:0;overflow:hidden;mso-hide:all;">{{{PREVIEW_TEXT}}}&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;</div>
        ...
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="600" class="container" style="max-width:600px;width:100%;background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 6px -1px rgba(0,0,0,0.1),0 2px 4px -1px rgba(0,0,0,0.06);">
          ...
          <tr>
            <td style="padding:0 40px 16px;" class="mobile-padding">
              ...
              <!-- Plain line first (survives clients that strip nested tables); banner below for emphasis. -->
              <p style="margin:16px 0 0;padding:0;font-size:17px;line-height:26px;color:#0f172a;font-family:Arial,Helvetica,sans-serif;font-weight:700;">Reservation reference: {{{RESERVATION_REFERENCE}}}</p>
              {{{RESERVATION_REFERENCE_HTML}}}
            </td>
          </tr>
          <tr>
              ...
              <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
                <tr>
                  <td>
                    ...
                    <p style="margin:0 0 8px;font-size:15px;line-height:22px;color:#0f172a;font-family:Arial,Helvetica,sans-serif;"><strong>Reservation reference:</strong> {{{RESERVATION_REFERENCE}}}</p>
              ...
              <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
                ...
                <tr>
                  <td align="center" style="font-size:13px;color:#9ca3af;line-height:1.6;font-family:Arial,Helvetica,sans-serif;">
                    ...
                    <p style="margin:0 0 8px;"><strong style="color:#e5e7eb;">Reservation reference:</strong> {{{RESERVATION_REFERENCE}}}</p>
                    <p style="margin:0;">
