...
export async function sendInvestorLeadEmail(lead) {
  ...
  const subject = stripHeaderSafe(`New investor proposal — ${name}`, 180);
  ...
  const html = `
    <div style="font-family:Georgia,serif;background:#f7f9f8;padding:28px 16px;">
      <div style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #e6ecea;padding:28px 32px;">
        ...
        <h1 style="margin:0 0 20px;font-size:22px;line-height:1.3;color:#0c1a1a;font-weight:500;">New investor proposal</h1>
        <table style="width:100%;border-collapse:collapse;">
          ${row("Name", name)}
          ${row("Email", lead.email)}
          ${row("Organization", lead.organization)}
          ${row("Role", lead.role)}
          ${row("Website", lead.firmUrl)}
          ${row("Proposal", lead.proposal)}
          ${row("Submitted", submitted)}
          ${row("Source", "Isisel /investors")}
        </table>
      </div>
    </div>
  `;
  ...
  const text = [
    "New investor proposal received.",
    "",
    `Name: ${name}`,
    `Email: ${lead.email}`,
    `Organization: ${lead.organization || "—"}`,
    `Role: ${lead.role || "—"}`,
    `Website: ${lead.firmUrl || "—"}`,
    `Proposal: ${lead.proposal || "—"}`,
    `Submitted: ${submitted}`,
    "Source: Isisel /investors",
  ].join("\n");
