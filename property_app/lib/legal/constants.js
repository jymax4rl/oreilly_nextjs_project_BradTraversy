/** Current Terms & Conditions version — bump when legal text materially changes. */
export const TERMS_VERSION = "kama-terms-v1.0-2026-08-31";

/** localStorage key for client-side terms acceptance. */
export const TERMS_ACCEPTANCE_KEY = "kama-terms-acceptance";

/** Cookie + localStorage key for UI language preference (en | fr). */
export const LANG_PREFERENCE_KEY = "kama-lang";

/** Cookie set when the visitor explicitly picks EN or FR. */
export const LANG_CHOICE_KEY = "kama-lang-choice";

export const LANG_COOKIE_MAX_AGE = 60 * 60 * 24 * 365;

export const SUPPORTED_LANGS = ["en", "fr"];
export const DEFAULT_LANG = "en";
